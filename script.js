...
class JogoDaVelha {
  ...
  status() {
    if (this.vencedor === "-") {
      ...
      return `🎮 Vez de ${this.jogadorAtual.simbolo}`;
    }
  }
}
...
class JogoDaVelhaDOM {
  ...
  #imprimirSimbolos() {
    ...
    for (let linha = 0; linha < qtdLinhas; linha++) {
      for (let coluna = 0; coluna < qtdColunas; coluna++) {
        ...
        let simbolo = tabuleiro[linha][coluna];
        posicoes[indiceDaInterface].innerText = simbolo || "";

        // Cores diferentes para X e O
        if (simbolo === "X") {
          posicoes[indiceDaInterface].style.color = "#ff6b35";
        } else if (simbolo === "O") {
          posicoes[indiceDaInterface].style.color = "#b794f6";
        }
      }
    }
  }
  ...
  #criarLinhaTabuleiro(linha, tamanho) {
    ...
    for (let coluna = 1; coluna <= tamanho; coluna++) {
      let classes = "posicao ";
      if (linha === 1) classes += "posicao-cima ";
      else if (linha === tamanho) classes += "posicao-baixo ";

      if (coluna === 1) classes += "posicao-esquerda ";
      else if (coluna === tamanho) classes += "posicao-direita ";
